	...
	<?php include("admin-menu.php"); ?>
       <div class="offer-right-div">
		...
		if(isset($_POST['submit'])){
			$id1=$_GET['id'];
			extract($_POST);
			//echo "UPDATE `fare_chart` SET `min_bill`='$min_bill',`min_kms`='$min_kms',`extra_kms`='$extra_kms',`free_waiting_mins`=$free_waiting_mins,`free_ride_mins`=$free_ride_mins,`waiting_charges`=$waiting_charges,`ride_charges`=$ride_charges WHERE id=$id1";exit();
	  $sql=mysql_query("UPDATE `fare_chart` SET `min_bill`='$min_bill',`min_kms`='$min_kms',`extra_kms`='$extra_kms',`free_waiting_mins`=$free_waiting_mins,`free_ride_mins`=$free_ride_mins, `waiting_charges`=$waiting_charges,`ride_charges`=$ride_charges WHERE id=$id1");
		  ...
		  function isNumberKey(evt){
            ...
            <div class="table2" style="width:80%;margin:0px auto;">
              <form class="upload-form" method="post" action="" name="f1" onSubmit="return validdata()">
				...
				<div class="upload-pro">
                  <label>Waiting time charges/Minute</label>
                ...
                <div class="upload-pro">
                  <label>Free waiting minutes(Mins)</label>
                  <input type="text" placeholder="Waiting time charges" name="free_waiting_mins" value="<?php echo $free_waiting_mins; ?>" onkeypress="return isNumberKey(event);"/>
                  <div class="clear"></div>
                </div>
				<div class="upload-pro">
                  <label>Ride time charges/Minute</label>
                ...
                <div class="upload-pro">
                  <label>Free Ride Minutes(Mins)</label>
                  <input type="text" placeholder="Ride time charges" name="free_ride_mins" value="<?php echo $free_ride_mins; ?>" onkeypress="return isNumberKey(event);"/>
                  <div class="clear"></div>
                </div>
